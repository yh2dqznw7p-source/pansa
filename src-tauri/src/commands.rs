...
pub fn register_user(
    ...
    if password.len() < 6 {
        return AuthResult {
            ok: false,
            message: Some("Пароль слишком короткий".into()),
            user: None,
        };
    }
    ...
    if store.users_by_email.contains_key(&email) {
        return AuthResult {
            ok: false,
            message: Some("Email уже используется".into()),
            user: None,
        };
    }
    let user = User {
...
pub fn login_user(email: String, password: String) -> AuthResult {
    ...
    let uid = match store.users_by_email.get(&email).cloned() {
        ...
        None => {
            return AuthResult {
                ok: false,
                message: Some("Пользователь не найден".into()),
                user: None,
            }
        }
    };
    ...
    if user.password_hash != hash_password(&password) {
        return AuthResult {
            ok: false,
            message: Some("Неверный пароль".into()),
            user: None,
        };
    }
    store.current_user_id = Some(user.id.clone());
...
pub async fn request_verification_code(email: String) -> Result<String, String> {
    let url = format!("http://127.0.0.1:{}/send-code", crate::server::LOCAL_PORT);
    let body = serde_json::json!({ "email": email });
    // use tauri's reqwest via ureq-free path: we already host the server in-process,
    // so call the storage directly to avoid a network round trip.
    let code = gen_code();
    {
        let mut store = STORE.write();
    ...
    println!("[tauri-cmd] send-code {} -> {} (via {})", email, code, url);
    let _ = body;
    Ok(code)
}
