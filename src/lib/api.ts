...
(() => {
  if (mem.chats.size === 0) {
    ["Общий чат", "Поддержка", "Новости"].forEach((title) => {
      ...
      mem.chats.set(id, { id, title, members: [], last_message_at: Math.floor(Date.now() / 1000) });
    });
  }
})();
...
async function browserFallback<T>(cmd: string, args?: any): Promise<T> {
  switch (cmd) {
    case "register_user": {
      ...
      const user: User = { id: crypto.randomUUID(), email, nickname, balance: 0, role: "user", created_at: nowSec() };
    ...
    case "login_user": {
      ...
      mem.currentUserId = id;
      return { ok: true, message: null, user: mem.users.get(id)! } as T;
    }
    case "logout_user": mem.currentUserId = null; return true as T;
    case "current_user": return (mem.currentUserId ? mem.users.get(mem.currentUserId) ?? null : null) as T;
    case "top_up": {
      const { amount } = args;
    ...
    case "verify_code": {
      ...
      for (let i = mem.codes.length - 1; i >= 0; i--) {
        const c = mem.codes[i];
        if (!c.consumed && c.email === email && c.code === code) { c.consumed = true; return true as T; }
      }
      return false as T;
    }
    case "list_chats": return Array.from(mem.chats.values()) as T;
    case "list_messages": { const { chatId } = args; return mem.messages.filter((m) => m.chat_id === chatId) as T; }
    case "send_message": {
      const { chatId, text } = args;
      if (!mem.currentUserId) throw "Не авторизован";
      const u = mem.users.get(mem.currentUserId)!;
      const msg: Message = { id: crypto.randomUUID(), chat_id: chatId, author_id: u.id, author_nickname: u.nickname, text, created_at: nowSec() };
    ...
    case "create_chat": {
      const { title } = args;
      const chat: Chat = { id: crypto.randomUUID(), title, members: mem.currentUserId ? [mem.currentUserId] : [], last_message_at: nowSec() };
      mem.chats.set(chat.id, chat);
      return chat as T;
    }
    case "list_complaints": return mem.complaints as T;
    case "submit_complaint": {
      const { target, reason } = args;
      if (!mem.currentUserId) throw "Не авторизован";
      const u = mem.users.get(mem.currentUserId)!;
      const c: Complaint = { id: crypto.randomUUID(), from_user_id: u.id, from_nickname: u.nickname, target, reason, status: "open", created_at: nowSec() };
    ...
    case "resolve_complaint": {
      ...
      if (c) { c.status = "resolved"; return true as T; }
      return false as T;
    }
    case "list_users": return Array.from(mem.users.values()) as T;
    case "assign_role": {
    ...
    case "open_support_window": window.open("/support.html", "_blank"); return undefined as T;
    default: throw new Error(`Unknown command ${cmd}`);
  }
}

export const api = {
  register: (email: string, nickname: string, password: string, password2: string) => invoke<AuthResult>("register_user", { email, nickname, password, password2 }),
  login: (email: string, password: string) => invoke<AuthResult>("login_user", { email, password }),
  ...
  verifyCode: (email: string, code: string) => invoke<boolean>("verify_code", { email, code }),
  listChats: () => invoke<Chat[]>("list_chats"),
  listMessages: (chatId: string) => invoke<Message[]>("list_messages", { chatId }),
  sendMessage: (chatId: string, text: string) => invoke<Message>("send_message", { chatId, text }),
  createChat: (title: string) => invoke<Chat>("create_chat", { title }),
  listComplaints: () => invoke<Complaint[]>("list_complaints"),
  submitComplaint: (target: string, reason: string) => invoke<Complaint>("submit_complaint", { target, reason }),
  resolveComplaint: (id: string) => invoke<boolean>("resolve_complaint", { id }),
  listUsers: () => invoke<User[]>("list_users"),
  assignRole: (userId: string, role: Role) => invoke<User>("assign_role", { userId, role }),
