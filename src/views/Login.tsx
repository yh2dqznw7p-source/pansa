...
export function Login() {
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      ...
      if (!r.ok || !r.user) {
        setErr(r.message ?? "Не удалось войти");
      } else {
        setUser(r.user);
      }
    } catch (e: any) {
  ...
  function oauthStub(provider: string) {
    setErr(`${provider}: требуется client_id в настройках провайдера`);
  }
    ...
    >
      <form className="auth-card lg-surface lg-surface--strong lg-refract" onSubmit={submit}>
        ...
        <div className="social-grid">
          <button type="button" className="lg-button" onClick={() => oauthStub("Google")}>
            <IconGoogle /> Google
          </button>
          <button type="button" className="lg-button" onClick={() => oauthStub("ВКонтакте")}>
            <IconVK /> ВКонтакте
          </button>
          <button type="button" className="lg-button" onClick={() => oauthStub("Apple")}>
            <IconApple /> Apple ID
          </button>
        </div>
