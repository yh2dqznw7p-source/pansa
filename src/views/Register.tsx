...
export function Register() {
  ...
  async function requestCode(e: React.FormEvent) {
    ...
    setBusy(true);
    try { const c = await api.requestCode(email.trim()); setSentCode(c); setStep("code"); }
    catch (e: any) { setErr(typeof e === "string" ? e : e?.message ?? "Ошибка"); }
  ...
  async function verifyAndRegister(e: React.FormEvent) {
    ...
    try {
      ...
      if (!ok) { setErr("Неверный код"); setBusy(false); return; }
    ...
    <motion.div className="auth-viewport" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -16 }} transition={{ duration: 0.35, ease: [0.22, 1, 0.36, 1] }}>
      <div className="auth-card lg-surface lg-surface--strong lg-refract">
        <div>
          ...
          <p className="auth-card__sub">{step === "form" ? "Заполните данные — мы отправим код на ваш локальный сервер" : "Введите код подтверждения"}</p>
        </div>
        {step === "form" && (
          <form className="form" onSubmit={requestCode}>
            ...
            <div className="form__field"><label className="label">Повторите пароль</label><input type="password" className={`lg-input ${mismatch ? "lg-input--error" : ""}`} value={pwd2} onChange={(e) => setPwd2(e.target.value)} required />{mismatch && <div className="error-hint">Пароли не совпадают</div>}</div>
            {err && <div className="error-hint">{err}</div>}
            <motion.button type="submit" className="lg-button lg-button--primary" whileTap={{ scale: 0.98 }} disabled={busy}>{busy ? "Отправка кода…" : "Отправить код"}</motion.button>
          </form>
        )}
        {step === "code" && (
          <form className="form" onSubmit={verifyAndRegister}>
            <div className="muted" style={{ fontSize: 13 }}>Код отправлен на локальный сервер <code>127.0.0.1:5005</code>.{sentCode && <> <span className="subtle">(dev: <b>{sentCode}</b>)</span></>}</div>
            <div className="form__field"><label className="label">Код подтверждения</label><input className="lg-input" inputMode="numeric" value={code} onChange={(e) => setCode(e.target.value.replace(/[^\d]/g, "").slice(0, 6))} required maxLength={6} /></div>
            {err && <div className="error-hint">{err}</div>}
            <div className="row" style={{ justifyContent: "space-between" }}>
              <button type="button" className="lg-button" onClick={() => setStep("form")}>Назад</button>
              <motion.button type="submit" className="lg-button lg-button--primary" whileTap={{ scale: 0.98 }} disabled={busy}>{busy ? "Проверка…" : "Подтвердить"}</motion.button>
            </div>
          </form>
        )}
        <div className="auth-card__link"><a href="#" onClick={(e) => { e.preventDefault(); setRoute("login"); }}>Уже есть аккаунт? Войти</a></div>
      </div>
    </motion.div>
  );
}
