...
export function Settings() {
  ...
  async function testConnection() {
    ...
    setPingMsg("");
    try {
      // Temporarily save so api.ping() uses the new value
      setServerUrl(server);
  ...
  return (
    <motion.div className="settings" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.35, ease: [0.22, 1, 0.36, 1] }}>
      <div className="settings__content">
        ...
        {/* Server — TOP, because nothing works without it */}
        <div className="lg-surface lg-refract card">
          <h3 className="card__title">Сервер</h3>
          <div className="subtle" style={{ fontSize: 12, marginBottom: 12 }}>
            Адрес вашего OffMessenger-сервера. По умолчанию локальный (127.0.0.1:5005). На продакшене укажите <code>https://your-domain.com</code>.
          </div>
          ...
          {ping === "ok" && <div className="muted" style={{ color: "var(--success)", marginTop: 8, fontSize: 13 }}>✓ подключено — {pingMsg}</div>}
          {ping === "fail" && <div className="error-hint" style={{ marginTop: 8 }}>✗ {pingMsg}</div>}
        </div>
        ...
        <div className="lg-surface lg-refract card">
          ...
          <div className="row">
            <div className="segmented">
              {themes.map((t) => (
                <button key={t.value} className={`segmented__item ${s.theme === t.value ? "segmented__item--active" : ""}`} onClick={() => s.setTheme(t.value)}>{t.label}</button>
              ))}
            </div>
          </div>
        </div>
